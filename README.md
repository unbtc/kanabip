...
3. **記入の順番決定**：
暗号表に打ち込む順番を決めます。**キーロガー等のマルウェアからニーモニックを守るために、順番はランダムかつダミーとなるワード(1～3単語、合計40単語程度)を間に挟んで入力する必要があります**。メモに書いた組み合わせの隣に適当な順番で12までの数字を書きます。（もしくは、記入したものから二重線で消していくのもいいでしょう。）`間違っても絶対に上から順に数字をかいたりしないで下さい。非常に危険です。`
